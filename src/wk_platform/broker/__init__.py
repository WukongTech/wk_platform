...
#        assert(False)
    ...
    def addExecutionInfo(self, orderExecutionInfo):
        ...
        if self.getRemaining() == 0:
            ...
            logger.getLogger("broker/__init__/addExecution").info("remaining is 0 ,change to Filled")
            self.switchState(Order.State.FILLED)
        else:
            """
            暂时注释，allOrNone模式下存在当前持仓不满足卖出量的情况
            """
            
            #assert(not self.__allOrNone)
            logger.getLogger("broker/__init__/addExecution").info("remaining is not 0 ,change to part-Filled")
...
class Broker(observer.Subject, metaclass=abc.ABCMeta):
    ...
    """
    notifyOrderEvent会触发strategy的onOrderUpdated函数
    """
