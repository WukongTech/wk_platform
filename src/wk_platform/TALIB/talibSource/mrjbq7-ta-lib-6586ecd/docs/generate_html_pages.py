...
def generate_groups_markdown():
    ...
    for group, group_docs in list(get_groups_markdown().items()):
...
def get_groups_markdown():
    ...
    ret = {}
    for group, funcs in list(talib.get_function_groups().items()):
        ...
        for func in funcs:
            ...
            if 'price' in inputs and 'prices' in inputs:
                ...
                input_names = ', '.join([x for x in list(inputs.values()) if x])

            params = ', '.join(
                ['%s=%i' % (param, default)
                 for param, default in list(f.info['parameters'].items())])
