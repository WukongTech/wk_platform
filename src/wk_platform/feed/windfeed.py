...
content: Windfeed的数据源封装
"""
import datetime
from pyalgotrade.barfeed import common
from pyalgotrade.utils import dt
from pyalgotrade.bar import Frequency
from wk_platform.feed import csvfeed
...
class RowParser(csvfeed.RowParser):
    def __init__(self, dailyBarTime, frequency, timezone=None, sanitize=False, barClass=bar.StockBar):
    ...
    def parseBar(self, csvRowDict):
        ...
        sec_name = str(csvRowDict["sec_name"])

        if self.__sanitize:
            open_, high, low, close = common.sanitize_ohlc(open_, high, low, close)
        return self.__barClass(dateTime, open_, high, low, close, volume, adjClose,new_column, trade_status,maxupordown,sec_name,self.__frequency)
...
class Feed(csvfeed.BarFeed):
    ...
    def __init__(self, frequency=Frequency.DAY, timezone=None, maxLen=None):
