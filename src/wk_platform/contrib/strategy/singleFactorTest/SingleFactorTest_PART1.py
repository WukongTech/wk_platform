...
class SingleFactorAnalyze():
    ...
    def MulticollinearityTest(self,datSelect,factorList,pastPeriodN):
                    ...
                    while k<N:
                        ...
                        if x[0] in corr_groups[k]:
                           ...
                           if sign1 > 1:
                               ...
                               for corr_one in corr_groups:
                                   addElement = corr_groups[k]
                                   if x[0] in corr_one:
                                       corr_one = corr_one.extend(addElement)
        ...
        print([[factorList[y] for y in x] for x in resultList])
    ...
    def readDataTech(self):
        
        print("data begin load at %s" %(time.strftime('[%Y-%m-%d %H:%M:%S]', time.localtime(time.time()))))
        
        self.__datVal  = pd.read_hdf("D://wk_bkt_public/platformData/tech_factors/stock_factors.h5")
    ...
    def readData(self):
         ...
         self.__datVal  = pd.read_hdf("D://wk_bkt_public/platformData/factorData.h5")
    ...
    def __selectDataColumnsTech(self):
        """
        为下面的合并做一些数据格式的处理
        """
        ...
        #print datSelect[['Date','windcode']]
     
        
        datForMerge = pd.read_hdf("D://wk_bkt_public/platformData/autoDailyData/dataForRegression.h5")
        datForMerge = datForMerge[['Date','windcode','Volume','future_logreturn','MV','industry_name']]
        datForMerge.rename(columns={'Volume':'volume'}, inplace=True)
  
    
        datForMerge['Date'] = pd.to_datetime(datForMerge['Date'],format = '%Y%m%d')
        #print datForMerge[['Date','windcode']]
        datSelect = pd.merge(datSelect, datForMerge,how = 'inner',on = ['Date','windcode'])
    ...
    def __resultplots(self,df1,items,factorName,types,freq):
        ...
        plt.savefig('Plots/' + factorName + "_" + types+ "_"+'AnalysisPlots.png')  
        #plt.show()                     
    ...
    def data_process(self, begin, end, freq,typeName):
        ...
        if typeName == 'fund':
            ...
            self.readDataTech() 
            datSelect = self.__selectDataColumnsTech()
            print(datSelect)
            if freq == 'month':
    ...
    def __singleFactorRegression(self, datSelect, factorList, freq,indexBench):
        ...
        for factorName in factorList:  
            ...
            ResultList.to_csv("dataFile/tempFile/ResultList.csv",encoding = 'gbk',index=False)
            
            df1=pd.read_csv("dataFile/tempFile/ResultList.csv", encoding = 'gbk')
            ...
            Return_allfactors.to_csv("dataFile/tempFile/Return_allfactors.csv", encoding = 'gbk',index=False)
            ...
            writer = pd.ExcelWriter('dataFile/PART1/singleFactor/singleFactorOutput_'+factorName+'_'+indexBench+'.xlsx', options = {'encoding' : 'utf-8'}, engine = 'xlsxwriter')
    ...
    def output(self,datSelect, factorList,freq,indexBench):
        
        
        """
        单因子测试，每个因子输出一个文件
        """
        self.__singleFactorRegression(datSelect,factorList,freq,indexBench)
        
        """
        周期/非周期性行业
        """
        self.__singleFactorRegression_classification(datSelect,factorList,'cycle',freq).to_csv('dataFile/PART1/singleFactor/singleFactorOutput_cycle.csv',encoding = 'gbk', index=False)
        ...
        """
        self.__singleFactorRegression_classification(datSelect,factorList,'ROE_TTM',freq).to_csv('dataFile/PART1/singleFactor/singleFactorOutput_ROEclass'+indexBench+'.csv',encoding = 'gbk', index=False)
        self.__singleFactorRegression_classification(datSelect,factorList,'MV',freq).to_csv('dataFile/PART1/singleFactor/singleFactorOutput_MVclass'+indexBench+'.csv',encoding = 'gbk', index=False)
    ...
    def outputIndex(self,datSelect, factorList,freq,indexBench):
...
def main():
    ...
    if indexBench == '':
        if typeName == 'original':
            ...
            datSelect = pd.read_hdf('dataFile/PART1/data/dataMonthly/datSelect_'+typeName+'.h5')
            print("end read data")
            print(datSelect.columns)
            ...
            SingleFactor.output(datSelect, factorList,freq,indexBench)
            ...
            for dataName in dataNameList:
                print(dataName)
                print("begin read data")
                datSelect = pd.read_hdf('dataFile/PART1/data/dataMonthly/datSelect_'+typeName+dataName+'.h5')
                print("end read data")
            ...
            factorList = ['ad20','ad6','amt_avg20','aroon','bbi','bias6','br','cr','dbcd','elder','mf20','mfi20','obv20','obv6','pvt12','pvt6','rsi24','trix','vema20','vstd10','vstd20']
            factors = [factorList]
            #execution(factors, periodList,typeName)
            #output(datSelect, factorList,freq)
         ...
         factorList = ['PB']
         datSelect = pd.read_csv('dataFile/PART1/dataIndex/data_monthly'+indexBench+'.csv', encoding = 'gbk')
            

         SingleFactor.outputIndex(datSelect, factorList,freq,indexBench)
