...
sys.path.append('D:\\wk_bkt_public\platform\\pyalgotrade_cn_wk')
sys.path.append('/home/wk_bkt_public/platform/pyalgotrade_cn_wk')
...
def loadDataFromHDF5(begin, end, localOrServer):
    ...
    if localOrServer == 0:
        datTotal = pd.read_hdf("D://wk_bkt_public/platformData/autoDailyData/priceDataForBackTest.h5")
        #datTotal = pd.read_hdf("C://platformData/autoDailyData/newlyPriceData_post.h5")
    else:
        datTotal = pd.read_hdf("/home/wk_bkt_public/platformData/autoDailyData/priceDataForBackTest.h5") 
    begin = str(begin)
    end = str(end)
...
def strategyProcess_post(feed,fileName,begin,end):
    ...
    实例化策略类
    """
    datVal = pd.read_csv('dataFile/PART2/weightFile/weightFileFinalOutput/%s'%(fileName), encoding = 'gbk')
    ...
    读取benchMark文件
    """
    #userBenchMark = pd.read_excel("benchMark/ZZ100.xlsx", encoding = 'gbk')
    #datVal = pd.read_excel("benchMark/SZ180.xlsx", encoding = 'gbk')
    #userBenchMark[u'日期'] =pd.to_datetime(userBenchMark[u'日期'], format = '%Y%m%d')
    #userBenchMark[u'日期'] = [pd.datetime.strftime(x, '%Y-%m-%d') for x in userBenchMark[u'日期']]
    
    output = strategyOutput.StrategyOutput(myStrategy, begin, end, 'indexEnhancement', 0.02)
    #output = strategyOutput.straOutput(myStrategy,begin,end, '300价值增强策略', 0.02)
    output.pre_process()
    myStrategy.run()
...
def main():
        ...
        begindate = '20050601'
        enddate = '20170914'
        print('singlefactor weight backtest, including slippage, all stocks after 2005 to 2017, after change')
        ...
        遍历文件夹下的所有文件
        """
        for root, dirs, files in os.walk("dataFile/PART2/weightFile/weightFileFinalOutput"):
            print(root)
            print(dirs)
            print(files)
            fileList = files
